...
server.listen(PORT, () => {
  ...
  // ================================================================
  // ✅ NAYA: "Returned to Factory" stale-delivery flush ab BACKGROUND
  // interval par automatically chalta hai — kisi admin/driver ke
  // page/API hit karne ka wait nahi karta. Pehle yeh sirf tab chalta
  // tha jab koi specific page (deliveries list/details, driver's
  // my-delivery) khola jaata tha — agar koi na khole to purani
  // (previous-day) delivery hamesha "Assigned"/"In_transit" hi dikhti
  // rehti thi, kabhi "Returned_to_Factory" nahi banti thi.
  //
  // Server start hote hi ek baar turant chalta hai (restart ke baad
  // turant catch-up ho jaaye), phir har 10 minute mein dobara chalta
  // rahega.
  // ================================================================
  const { flushAllStaleDeliveriesGlobally } = require('./controllers/Driver/deliveryController');

  // ⏱ 15 second delay taaki MongoDB connection (connectDB() upar async
  // hai) fully ready ho jaaye pehle isse chalne se pehle
  setTimeout(() => {
    flushAllStaleDeliveriesGlobally();
  }, 15000);
