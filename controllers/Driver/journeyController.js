...
// // ==================== GET MY DELIVERY QUEUE (proximity sorted) ====================
// exports.getMyDeliveryQueue = async (req, res) => {
//   try {
//     const driver = req.user;
//     if (!driver) return errorResponse(res, 'Driver not authenticated', 401);

//     // In-memory live location try karo (socket se), warna DB fallback getSortedDeliveryQueueForDriver ke andar hoga
//     const io = getIO(req);
//     const liveLoc = io?.driverLocations?.get(driver._id.toString());
...
//     return successResponse(res, 'Delivery queue sorted by proximity', {
//       nextDelivery: sortedQueue[0] || null,
//       queue: sortedQueue,
//       totalActive: sortedQueue.length,
//     });
//   } catch (error) {
//     console.error('Get My Delivery Queue Error:', error);
//     return errorResponse(res, error.message || 'Failed to get delivery queue', 500);
//   }
// };
...
exports.uploadProofSignature = async (req, res) => {
  try {
    ...
    if (delivery.orderId) {
      try {
        ...
        if (updatedOrder) {
          ...
          // ✅ Order delivered ho gaya — agar koi admin edit page khole baithe hai to use turant lock karo
          const lockIo = req.app.get('io');
          if (lockIo) {
            lockIo.to('admin-room').emit('order:locked', {
              orderId: updatedOrder._id.toString(),
              status: 'delivered',
              message: 'This order has been delivered and can no longer be edited.',
              timestamp: new Date().toISOString()
            });
          }
        } else {
    ...
    // ✅ ROUTE CHAINING FIX: Chained route ka agla delivery activate karo.
    // Previous delivery (Point A→B) complete hui, ab uska destination (B)
    // agli delivery (B→C) ka pickup ban jaata hai — factory wapas nahi dikhega.
    if (delivery.nextDeliveryId) {
      try {
        const nextDelivery = await Delivery.findById(delivery.nextDeliveryId);
        if (nextDelivery && nextDelivery.status === 'Pending') {
          nextDelivery.pickupLocation = delivery.deliveryLocation;
          nextDelivery.status = 'assigned';
          nextDelivery.scheduledPickupTime = new Date();
          await nextDelivery.save();

          if (nextDelivery.orderId) {
            const nextOrderQuery = mongoose.Types.ObjectId.isValid(nextDelivery.orderId)
              ? { _id: nextDelivery.orderId }
              : { orderNumber: nextDelivery.orderId };
            await Order.findOneAndUpdate(nextOrderQuery, { status: 'assigned' });
          }

          const chainIo = req.app.get('io');
          if (chainIo) {
            chainIo.to(`driver-${req.user._id}`).emit('route:next-delivery-ready', {
              deliveryId: nextDelivery._id.toString(),
              routeGroupId: nextDelivery.routeGroupId,
              sequence: nextDelivery.routeSequence,
              pickupLocation: nextDelivery.pickupLocation,
              deliveryLocation: nextDelivery.deliveryLocation,
              message: `Stop ${nextDelivery.routeSequence} ready — pickup from previous delivery point`,
              timestamp: new Date().toISOString()
            });
          }
          console.log(`✅ Next delivery in route activated | deliveryId: ${nextDelivery._id}`);
        }
      } catch (chainErr) {
        console.error('❌ Route chaining failed:', chainErr.message);
      }
    }
