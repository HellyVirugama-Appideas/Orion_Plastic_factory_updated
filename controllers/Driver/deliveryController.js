...
// const isOldDelivery = (d) => {
//   const today = startOfToday();
//   // Prefer scheduledDeliveryTime, fallback to updatedAt / createdAt
//   if (d.scheduledDeliveryTime) {
//     return new Date(d.scheduledDeliveryTime) < today;
//   }
//   // No schedule → use updatedAt (assignment time) or createdAt
//   const refDate = d.updatedAt || d.createdAt;
//   return refDate ? new Date(refDate) < today : true;
// };

// /**
//  * Old pending / assigned deliveries ne flush kari de:
//  * - driverId = null
//  * - status = 'Pending' (admin phari assign kari sake)
//  * - journey / route fields clear
//  */
// async function flushOldDeliveriesForDriver(driverId) {
//   try {
//     const today = startOfToday();

//     // Sirf upcoming statuses + old date vali
//     const oldOnes = await Delivery.find({
//       driverId,
//       status: { $in: UPCOMING_STATUSES },
//       $or: [
//         { scheduledDeliveryTime: { $lt: today } },
//         {
...
//         }
//       ]
//     }).select('_id trackingNumber status scheduledDeliveryTime');

//     if (oldOnes.length === 0) {
//       console.log(`[FLUSH] No old deliveries to flush for driver ${driverId}`);
//       return 0;
//     }
...
//     console.log(
//       `[FLUSH] ✅ Flushed ${result.modifiedCount} old delivery(ies) for driver ${driverId}:`,
//       oldOnes.map(d => d.trackingNumber).join(', ')
//     );
//     return result.modifiedCount;
//   } catch (err) {
//     console.error('[FLUSH] Error while flushing old deliveries:', err.message);
//     return 0;
//   }
// }
...
//   await flushOldDeliveriesForDriver(driverId);
...
//   // ✅ Diagnostic: chahe driver GPS use ho ya fallback, final resolved
//   // starting point hamesha yahan clearly log hoga — agla bug isi line se
//   // turant pakad mein aayega.
//   console.log(
...
//     driverId,
//     // Optional extra safety: scheduled today ya baad ma, athva aaj update thayeli
//     $or: [
//       { scheduledDeliveryTime: { $gte: today } },
//       { scheduledDeliveryTime: null, updatedAt: { $gte: today } },
//       { scheduledDeliveryTime: null, createdAt: { $gte: today } }
//     ]
//   })
