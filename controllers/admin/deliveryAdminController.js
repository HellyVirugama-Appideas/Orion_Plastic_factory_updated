...
exports.createDeliveryFromOrder = async (req, res) => {
  try {
    ...
    const {
      ...
      routeDistance,
      routeDuration,
      chainPickup // ✅ NEW: admin ne manually "chain karo" checkbox tick kiya hai ya nahi
    } = req.body;

    // Checkbox se "on"/"true"/"1" jaisi values aa sakti hain — sabko boolean bana do
    const shouldChainPickup = chainPickup === 'on' || chainPickup === 'true' || chainPickup === true || chainPickup === '1';
    ...
    // ================================================================
    // ✅ CHAINING (OPT-IN ONLY): pehle ye HAMESHA automatically apply ho
    // jaata tha — jisse admin ne jo bhi pickup diya ho, wo ignore hoke
    // driver ki pichli delivery ka drop point pickup ban jaata tha (chahe
    // admin ne factory ya koi aur asli pickup address diya ho).
    //
    // Ab: chaining SIRF tab apply hogi jab admin form pe "chain karo"
    // checkbox manually tick kare (shouldChainPickup === true). Warna
    // hamesha order.pickupLocation (jo admin ne is order ke liye diya
    // hai) hi use hoga — first delivery ho ya nahi, koi farak nahi padta.
    // ================================================================
    let effectivePickupLocation = order.pickupLocation;
    let previousDeliveryForDriver = null; // traceability ke liye bahar declare kiya, taaki neeche previousDeliveryId mein use ho sake
    let hasValidPreviousStop = false; // ✅ FIX: ye bhi bahar declare kiya — neeche "remarks" mein bhi use hota hai

    if (shouldChainPickup) {
      previousDeliveryForDriver = await Delivery.findOne({ driverId })
        .sort({ createdAt: -1 })
        .select('deliveryLocation trackingNumber');

      hasValidPreviousStop = !!(
        previousDeliveryForDriver?.deliveryLocation?.coordinates?.latitude &&
        previousDeliveryForDriver?.deliveryLocation?.coordinates?.longitude &&
        previousDeliveryForDriver?.deliveryLocation?.address
      );
      ...
      if (hasValidPreviousStop) {
        effectivePickupLocation = previousDeliveryForDriver.deliveryLocation;
        console.log(`[CREATE-DELIVERY] 🔗 Chaining manually enabled — using previous delivery's destination as pickup: "${previousDeliveryForDriver.deliveryLocation.address}" (from ${previousDeliveryForDriver.trackingNumber})`);
      } else {
        console.log(`[CREATE-DELIVERY] Chaining enabled, but koi valid previous delivery nahi mili — order ka apna pickup use hoga`);
      }
    } else {
      console.log(`[CREATE-DELIVERY] Chaining nahi select ki gayi — order ka apna pickup (${order.pickupLocation?.address || 'N/A'}) use hoga`);
    }
