...
// CSV IMPORT STORAGE (Customers bulk import)
const csvStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const dir = 'public/uploads/csv-imports/';
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    cb(null, dir);
  },
  filename: (req, file, cb) => {
...
const allowCsvOnly = (req, file, cb) => {
  const allowed = /csv/;
  const ext = allowed.test(path.extname(file.originalname).toLowerCase());
  const mime = ['text/csv', 'application/vnd.ms-excel', 'text/plain', 'application/octet-stream'].includes(file.mimetype);
  if (ext) cb(null, true);
...
module.exports = {
  ...
  uploadCustomerCsv: uploadCsv.single('csvFile'),
